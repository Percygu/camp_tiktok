...
func PublishAction(ctx *gin.Context) {
	...
	publish, err := utils.GetVideoSvrClient().PublishVideo(ctx, &pb.PublishVideoRequest{
		UserId:   userId.(int64),
		Title:    title,
		SaveFile: saveFile,
	})
...
func GetPublishList(ctx *gin.Context) {
	...
	resp, err := utils.GetVideoSvrClient().GetPublishVideoList(ctx, &pb.GetPublishVideoListRequest{
